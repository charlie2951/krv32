...
// =====================================================
// Internal Helpers (Replaced Burst with Sequential Reads)
// =====================================================

// Reads two consecutive registers to form a 16-bit Little Endian value
static uint16_t read16(i2c_t *i2c, uint8_t reg) {
    uint8_t low  = i2c_read(i2c, BME280_ADDR, reg);
    uint8_t high = i2c_read(i2c, BME280_ADDR, reg + 1);
    return (uint16_t)(low | (high << 8));
}

static int16_t readS16(i2c_t *i2c, uint8_t reg) {
    return (int16_t)read16(i2c, reg);
}
...
int8_t bme280_init_i2c(i2c_t *i2c) {
    // Single byte read for Chip ID
    uint8_t id = i2c_read(i2c, BME280_ADDR, BME280_CHIP_ID);
    printf("chip id=%x\r\n",id);
    if (id != 0x58 && id != 0x60) return -1;

    // Load Calibration (Individual 16-bit reads)
    calib.dig_T1 = read16(i2c, 0x88);
    calib.dig_T2 = readS16(i2c, 0x8A);
    calib.dig_T3 = readS16(i2c, 0x8C);
    
    calib.dig_P1 = read16(i2c, 0x8E);
    calib.dig_P2 = readS16(i2c, 0x90);
    calib.dig_P3 = readS16(i2c, 0x92);
    calib.dig_P4 = readS16(i2c, 0x94);
    calib.dig_P5 = readS16(i2c, 0x96);
    calib.dig_P6 = readS16(i2c, 0x98);
    calib.dig_P7 = readS16(i2c, 0x9A);
    calib.dig_P8 = readS16(i2c, 0x9C);
    calib.dig_P9 = readS16(i2c, 0x9E);

    calib.dig_H1 = i2c_read(i2c, BME280_ADDR, 0xA1);
    calib.dig_H2 = readS16(i2c, 0xE1);
    calib.dig_H3 = i2c_read(i2c, BME280_ADDR, 0xE3);
    
    // H4 and H5 are special bit-packed registers across 0xE4, 0xE5, 0xE6
    uint8_t e4 = i2c_read(i2c, BME280_ADDR, 0xE4);
    uint8_t e5 = i2c_read(i2c, BME280_ADDR, 0xE5);
    uint8_t e6 = i2c_read(i2c, BME280_ADDR, 0xE6);
    calib.dig_H4 = (int16_t)((e4 << 4) | (e5 & 0x0F));
    calib.dig_H5 = (int16_t)((e6 << 4) | (e5 >> 4));
    
    calib.dig_H6 = (int8_t)i2c_read(i2c, BME280_ADDR, 0xE7);

    // Configuration writes (Already single byte)
    i2c_write(i2c, BME280_ADDR, BME280_REG_CTRL_HUM, 0x01);
    i2c_write(i2c, BME280_ADDR, BME280_REG_CTRL_MEAS, 0x27);//for normal mode
   // i2c_write(i2c, BME280_ADDR, BME280_REG_CTRL_MEAS, 0x24); //forced mode, Oversampling x1, Sleep Mode (bits 1:0 = 00)
    return 0;
}

void bme280_i2c_trigger_forced(i2c_t *i2c) {
    // Write 0x25: 
    // Bits [7:5] Temp x1, [4:2] Press x1, [1:0] Forced Mode (01)
    i2c_write(i2c, BME280_ADDR, BME280_REG_CTRL_MEAS, 0x25);
    
    // Wait for measurement to complete (Busy bit in Status Register 0xF3)
    // Bit 3 is 'measuring' bit.
    while (i2c_read(i2c, BME280_ADDR, 0xF3) & 0x08);
}

// =====================================================
// Measurement Functions (Re-implemented for Single Read)
// =====================================================

void bme280_i2c_get_temperature(i2c_t *i2c, int32_t *temp) {
    // Read 3 bytes for 20-bit Temperature (MSB, LSB, XLSB)
    uint8_t msb  = i2c_read(i2c, BME280_ADDR, 0xFA);
    uint8_t lsb  = i2c_read(i2c, BME280_ADDR, 0xFB);
    uint8_t xlsb = i2c_read(i2c, BME280_ADDR, 0xFC);
...
void bme280_i2c_get_pressure(i2c_t *i2c, uint32_t *press) {
    int32_t dummy; bme280_i2c_get_temperature(i2c, &dummy); // Updates t_fine

    // Read 3 bytes for 20-bit Pressure (MSB, LSB, XLSB)
    uint8_t msb  = i2c_read(i2c, BME280_ADDR, 0xF7);
    uint8_t lsb  = i2c_read(i2c, BME280_ADDR, 0xF8);
    uint8_t xlsb = i2c_read(i2c, BME280_ADDR, 0xF9);
...
void bme280_i2c_get_humidity(i2c_t *i2c, uint32_t *hum) {
    ...
    // Read 2 bytes for 16-bit Humidity (MSB, LSB)
    uint8_t msb = i2c_read(i2c, BME280_ADDR, 0xFD);
    uint8_t lsb = i2c_read(i2c, BME280_ADDR, 0xFE);
...
// =====================================================
// Legacy Wrapper (To support your existing test files)
// =====================================================
void bme280_i2c_read_fixed(i2c_t *i2c, int32_t *temp, uint32_t *press, uint32_t *hum) {
    // 1. Trigger the single measurement
    bme280_i2c_trigger_forced(i2c);

    // 2. Read the results (registers are now updated and chip is back to sleep)
    bme280_i2c_get_temperature(i2c, temp);
    bme280_i2c_get_pressure(i2c, press);
    bme280_i2c_get_humidity(i2c, hum);
}
