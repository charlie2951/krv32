...
module top(
    ...
    inout i2c2_sda,//pins for i2c2(SDA)-internal temp sensor
    inout i2c2_scl, //pins for i2c2(SCL)-internal temp sensor
    output [7:0] an,
    output [7:0] seg,
    ...
    output wire MOSI,
    input  wire MISO,
    //SPI master for onboard ADXL sensor(internal only)
    output wire SS1,
    output wire SCK1,
    output wire MOSI1,
    input  wire MISO1,
    ...
    //PLL status indicator LED
    output pll_led
  );

  assign pll_led = pll_locked; // Solid green means your clock is good
  ...
  wire [31:0] mem_rdata, mem_wdata, addr,segment_data,SPI0_DATA,SPI1_DATA, SPI_SYS_DATA;
  wire rstrb;
  wire [3:0] wr_strobe;
  //peripheral data collect wires
  wire [31:0] timer0_rdata, timer1_rdata;
  wire [31:0] i2c0_rdata, i2c1_rdata, i2c2_rdata;
  ...
  wire isI2CBOARD = (addr[31:8]==24'hC00002); //i2c board
  ...
  wire isSPI1 = (addr[31:16]==16'hC300);//SPI-1 master board sensor
  ...
  //I2C board mmio wrapper for internal temp sensor interfacing
  i2c_board_mmio i2c2(
                   .clk(clk),
                   .rst_n(rst),
                   .addr(addr),
  ...
  // SPI-1 Master board sensor ADXL interface
  riscv_spi1_wrapper spi1(
                       .clk(clk),
                       .reset(!rst),
                       .addr(addr),
                       .data_in(mem_wdata),
