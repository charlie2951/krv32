...
module cpu(
    input rst, clk,
    input boot_en,
    input [31:0] mem_rdata,//32 bit data from memory
...
*/
//Replacing ALU with procedural block - always @(*)
always @(*) begin
    if((isStype | isLtype | isJALR))
    alu_result =  ADD;
else
    case(funct3)
    3'b000: begin
        if((isRtype & ~funct7[5])|(isItype))
            alu_result =  ADD;
        else if(~(isStype|isLtype|isJALR) & funct7[5])
            alu_result = SUB[31:0];
    end
    3'b001: begin
            if(!isStype)
...
assign mem_wstrb = {4{(state==WAIT_LOADING) & isStype}} & STORE_wmask;
  ...
  begin
    ...
    state=0;
    data=0;
    addr = boot_en ? 32'hA0000000 : 32'hD0000000;//need to check
  ...
  begin
    if(rst)
    begin
      addr <= boot_en ? 32'hA0000000 : 32'hD0000000;//upon reset it will go to start memory
      state <= RESET;
      data <= 32'h00000000;
    end
    else
